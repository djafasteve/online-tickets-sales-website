...
import fr.afcepf.al27.ticketshack.front.util.MbMail;
...
public class MBPanier implements Serializable {
	...
	private MbMail mbMail;

	public MbMail getMbMail() {
		return mbMail;
	}

	public void setMbMail(MbMail mbMail) {
		this.mbMail = mbMail;
	}
	...
	public String validerPaiement() {
		...
		String url = "http://192.168.102.125:9090/ode/processes/processTicketsHack?wsdl";
		...
		bp.getRequestContext().put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY,
				"http://192.168.102.125:9090/ode/processes/processTicketsHack");
		...
		case "payer":
			...
			cmd.setLigneCommandes(listeLigneCommande);
			cmd.setReference(String.valueOf("dqgecmdddamgmje".hashCode()));
			...
			mbMail.sendFacture(cmd);
			listeLigneCommande.clear();
			page = "confirmation.xhtml?faces-redirect=true";
			break;
		...
		case "nonvalider":
			// carte Invalide
			paiementEchec = true;
			// pas de redirection page car avec Ajax qui prend la valeur du
			// boolean et rafraichir la page soit meme.
			break;
		...
		default:
			...
			for (Object b2 : listOccurences) {
				 ...
				 * ligneCommande.getTarif().getOccurence().setIdOccurence(b2.
				 * getId_occurence());
				 * ligneCommande.getTarif().getOccurence().setNombrePlaces(b2.
				 * getNbPlaces()); listeLigneCommande.add(ligneCommande);
				 * System.out.println("********lignecommande**********" +
				 * ligneCommande);
				 * 
				 * System.out.println("size list ligne commande******" +
				 * listeLigneCommande.size());
				 * 
				 * //isNotDispo = true;
				 */
			}

...
public class ImplDaoCommande implements IDaoCommande {
	...
	public List<Commande> findByUtilisateur(Utilisateur utilisateur) {
		String requete = "SELECT c FROM Commande c where c.utilisateur.idUtilisateur = :idUtilisateur";
	...
	@Override
	public Commande findLastCmdByUser(Utilisateur utilisateur) {
		Commande cmd = new Commande();
		String requete = "SELECT c FROM Commande c where c.utilisateur.idUtilisateur =:utilisateur ORDER BY c.dateCreation DESC";
		TypedQuery<Commande> query = em.createQuery(requete, Commande.class);
		query.setParameter("utilisateur", utilisateur.getIdUtilisateur());
		List<Commande> list = query.getResultList();
		if(!list.isEmpty()){
			cmd = list.get(list.size()-1);
		}
